...
**The set is registered, and that is the load-bearing part.** It sits in the discovery layer's parameter object and reaches its hash. A value that changes what the fence refuses whilst living in a module constant cannot be attributed to a registration: the same corpus would give two different refusal sets under one hash, and a row added to widen what a sweep lets through would leave nothing on the record. Adding a stopword is therefore a re-stamp, which is the cost and also the point. *The lexicon itself does not yet reach the hash, which is the same defect one level up; it is recorded here rather than fixed silently.*
...
| 21 | **Entity-fence error rates** *(revised, P75; corrected, P77 and P79)* | 200 hand-labelled proposals | **Two arms, of two kinds, never combined and never reported as one.** The **drawn** arm gives a rate: the share of clean class-level mechanisms the fence refuses, divided by its own n and never by the union of the arms. The **authored probe** arm gives coverage: which named routes into the fence are closed and which are left open, with **no percentage**, because probes are chosen rather than sampled and a proportion over a chosen set estimates nothing. A drawn episode-level sample would yield a second rate; until one exists, this arm is coverage. Each arm refuses to score where it is empty. **A provisional reading exists**, against the committed labelled set at `docs/labelled_proposals.json`, being 42 subjects (36 class-level mechanisms drawn by agent sweep, 6 authored probes), and the US security master: **0 of 36 refused (0%)** on the drawn arm, and **5 of 6 routes closed, the open one a title-case bare ticker**, which is the residual P76's rule takes on knowingly. On the fence as v1.13 left it the same 36 gave **3 refused (8%)**, on `Note`, `T` and `It`. **PROVISIONAL in two respects, both of which must clear before this is a calibration.** First, 42 subjects against the 200 this row specifies. Second, **the labels carry provenance `model_clerk`, not hand labels**: they are the clerk's classifications against the fixed taxonomy, and **the operator has not ratified them**. Recorded as a reading, and a reading is not the calibration | Design segment |
