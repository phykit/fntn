...
- Assume a value for any pending §13 row. **§13 row 1 CLOSED on 27 August 2026** on a published schedule read at per-trade granularity, and **the contracting-entity gap was RETIRED BY BEING SHOWN IMMATERIAL**: both IBKR entities publish byte-identical figures for every component the row uses. **Binding-path steps 1 and 3 are CLOSED, the first ever to close.** The election is **FIXED**, measured rather than fitted: fixed absorbs exchange, clearing and pass-through fees and tiered passes them through, and §4.3's next-open fill convention puts entries at the **opening auction** and stop exits at **remove liquidity**. **The clip floor DERIVES and is a FUNCTION OF SHARE PRICE, never a constant**: USD 6,155 at USD 43.79 a share, USD 8,522 at USD 20, and `clip_floor_unreachable_at_any_size` at or below USD 10.42. ***And there are TWO screens: the binding one is §6.7's cap, not the cost.*** Below **USD 13.20** a share the floor exceeds the largest position §6.7 will size, so between USD 10.42 and 13.20 a name is cost-reachable and size-unreachable. *The book is NOT empty: GBP 4,771 sits inside §6.7's GBP 1,875 to 15,000 band for any USD/GBP rate between 0.410 and 3.283.* **10 bp is a US-ONLY decision** and **AIM fails on COMMISSION, not tax**; **12.5 bp is where AIM becomes reachable**. **Row 1's SCOPE stays with the operator**, and under option A the other cost components are read, needed, and on no row.
